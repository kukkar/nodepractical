...
function resolveQuery(arr, resCB){
  result = [];
...
async.series([
    ...
    function(callback){
        ...
        resolveQuery(arr,function(){
         ...
         response.pieChart=[{state:result[0].state,count:result[0].count},{state:result[1].state,count:result[1].count},{state:result[2].state,count:result[2].count}];
           callback();
           });
       }
       ],function(err){
